...
1. vOTU IDs are determined by the `map-aniclst.tsv` file.
2. VC IDs are determined by the `vcontact3-final_assignments.csv` file.
...
```{r arg}
df_arg <- fread(here(path_data, "anno_abricate.tsv.gz")) %>% 
  dplyr::rename(protein_id = SEQUENCE) %>% 
  dplyr::mutate(contig_id = str_replace_all(protein_id, "-.*", "")) %>% 
  dplyr::select(c(contig_id, everything()))

DBI::dbWriteTable(con, "anno_prot_ARG", df_arg, overwrite = TRUE, row.names = FALSE)
```
